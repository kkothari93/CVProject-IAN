...
class fooler():
    ...
    def imgradRGB(self, c1, r1, c2, r2, RGB, z):
        ...
        # grad = tf.gradients([expr], [self.model.mean])
        grad = tf.gradients([expr], [self.model.latent])

        x_hat_eval, grad_out = self.sess.run(
            [x_hat, grad], feed_dict={self.model.orig: self.X,
                             self.model.Z: z,
                             self.model.EPS_BS: 1,
                             self.model.TRAIN: False})
        print(x_hat_eval.shape)
        imsave('x_hat.png', x_hat_eval[0])
...
def update_photo(data=None, widget=None):
    ...
    if data is None:  # By default, assume we're updating with the current value of Z
        data = np.repeat(np.repeat(np.uint8(
            unnorm_one(model.sample_at(np.float32([Z.flatten()]))[0])), 4, 1), 4, 2)
        # data = np.repeat(np.repeat(np.uint8(
...
def paint(event):
    ...
    # Else, update photo
        ...
        DELTA = model.sample_at(np.float32([Z.flatten()]))[
            0] - np.float32(RECON)
        # DELTA = model.sample_at(np.float32([Z.flatten()]))[
            # 0]-norm_one(np.float32(RECON))
        MASK = scipy.ndimage.filters.gaussian_filter(
            np.min([np.mean(np.abs(DELTA), axis=0), np.ones((64, 64))], axis=0), 0.7)
        # MASK = scipy.ndimage.filters.gaussian_filter(
            # np.min([np.mean(np.abs(DELTA), axis=0), np.ones((64, 64))], axis=0), 0.7)
        # MASK = (MASK-np.min(MASK))*1.0/(np.max(MASK)-np.min(MASK))
        # D = dampen(to_tanh(np.float32(RECON)),MASK*DELTA+(1-MASK)*ERROR)
        D = MASK*DELTA+(1-MASK)*ERROR

        # IM = np.uint8(0.5*unnorm_one(norm_one(RECON)+norm_one(D)))

        IM = (RECON-np.min(RECON))*1.0/(np.max(RECON)-np.min(RECON))  
        IM += D  # (D-np.min(D))*1.0/(np.max(D)-np.min(D))  
        # IM = RECON + D
        IM = (IM-np.min(IM))*1.0/(np.max(IM)-np.min(IM))
        IM = np.uint8(255.0*IM)

        imsave('Recon.png', RECON.swapaxes(1,0).swapaxes(1,2))
        print('RECON', np.min(RECON), np.max(RECON))
        imsave('Delta.png', DELTA.swapaxes(1,0).swapaxes(1,2))
        print('DELTA', np.min(DELTA), np.max(DELTA))
        imsave('Mask.png', MASK)
        print('Mask', np.min(MASK), np.max(MASK))
        imsave('D.png', D.swapaxes(1, 0).swapaxes(1,2))
        print('RECON', np.min(RECON), np.max(RECON))
        imsave('IM.png', IM.swapaxes(1,0).swapaxes(1,2))
        imsave('recond.png', (RECON+D).swapaxes(1,0).swapaxes(1,2))
        ...
        # IM = np.uint8(unnorm_one(norm_one(RECON)+D-np.min(D)))

        # Pass updates
        update_canvas(w)
        update_photo(IM, output)
        IM = np.float32(IM*1./256.)
...
def infer():
    ...
    s = model.encode_images(np.asarray([norm_one(IM)], dtype=np.float32))
    Z = np.reshape(s[0], np.shape(Z))

    # Get reconstruction
    RECON = model.sample_at(np.float32([Z.flatten()]))[0]
    # Get error
    ERROR = norm_one(np.float32(IM)) - np.float32(RECON)
...
def paint_latents(event):
    ...
    if SAMPLE_FLAG:
        ...
        # IM = np.uint8(unnorm_one(norm_one(RECON)+norm_one(D)))

        IM = (RECON-np.min(RECON))*1.0/(np.max(RECON)-np.min(RECON))
        IM += (D-np.min(D))*1.0/(np.max(D)-np.min(D))
        IM = (IM-np.min(IM))*1.0/(np.max(IM)-np.min(IM))
        IM = np.uint8(255.0*IM)

        imsave('Recon.png', RECON.swapaxes(1,0).swapaxes(1,2))
        imsave('Delta.png', DELTA.swapaxes(1,0).swapaxes(1,2))
        imsave('Error.png', ERROR.swapaxes(1,0).swapaxes(1,2))
        imsave('Mask.png', MASK)
        imsave('D.png', D.swapaxes(1, 0).swapaxes(1,2))
        imsave('IM.png', IM.swapaxes(1,0).swapaxes(1,2))
        imsave('recond.png', (RECON+D).swapaxes(1,0).swapaxes(1,2))
...
def Reset():
    ...
    Z = np.reshape(model.encode_images(np.asarray(
        [norm_one(IM)], dtype=np.float32))[0], np.shape(Z))
