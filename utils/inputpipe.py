...
def read_parse_preproc_mat(filename_queue):
    ''' read, parse, and preproc single example. '''

    with tf.variable_scope('read_parse_preproc_mat'):
        reader = tf.TFRecordReader()
        key, records = reader.read(filename_queue)
        ...
        features = tf.parse_single_example(
            ...
            features={
                ...
                "Pinv": tf.FixedLenFeature([], tf.string)
            }
        )

        P = tf.decode_raw(features["P"], tf.float32)
        Pinv = tf.decode_raw(features["Pinv"], tf.float32)
        P = tf.reshape(P, [50, 128**2])
        Pinv = tf.reshape(Pinv, [128**2, 50])
...
def get_batch(tfrecords_list, batch_size, shuffle=False, num_threads=1, min_after_dequeue=None, num_epochs=None):
    ...
    with tf.variable_scope(name):
        filename_queue = tf.train.string_input_producer(
            tfrecords_list, shuffle=shuffle, num_epochs=num_epochs)
...
def get_batch_join(tfrecords_list, batch_size, shuffle=False, num_threads=1, min_after_dequeue=None, num_epochs=None):
    ...
    with tf.variable_scope(name):
        filename_queue = tf.train.string_input_producer(
            tfrecords_list, shuffle=shuffle, num_epochs=num_epochs)
        example_list = [read_parse_preproc(
            filename_queue) for _ in range(num_threads)]
        ...
        if shuffle:
            ...
            batch = tf.train.batch_join(
                example_list, batch_size, capacity=capacity, allow_smaller_final_batch=True)

        return batch


def get_batch_join_mat(tfrecords_list, batch_size, shuffle=False, num_threads=1, min_after_dequeue=None, num_epochs=None):
    name = "batch_join" if not shuffle else "shuffle_batch_join"
    with tf.variable_scope(name):
        filename_queue = tf.train.string_input_producer(
            tfrecords_list, shuffle=shuffle, num_epochs=num_epochs)
        example_list = [read_parse_preproc_mat(
            filename_queue) for _ in range(num_threads)]
